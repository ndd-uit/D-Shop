...
import { ExternalLink, LoaderCircle, RefreshCw, RotateCcw, Search, WalletCards } from "lucide-react"
...
const STATUS_META = {
    PENDING: ["Đang xử lý", "border-[#e8c98e] bg-[#fff0cf] text-[#876022]"],
...
function ManagerRefundsPage() {
    ...
    const retry = async (refund) => {
        ...
        try {
            ...
            openGateway(result)
            toast.success("Đã gửi lại yêu cầu hoàn tiền")
    ...
    return <div className="mx-auto flex min-h-screen max-w-[1920px] bg-[#faf6ef] text-[#453c38]">
        ...
        <main className="min-w-0 flex-1">
            ...
            <div className="mx-auto max-w-[1440px] space-y-6 px-5 py-7 sm:px-8 lg:px-12 lg:py-9 xl:px-16">
                ...
                <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[["Đang xử lý", counts.pending], ["Thất bại", counts.failed], ["Thành công", counts.succeeded], ["Cần đối soát", counts.needsReconciliation]].map(([label, value]) => <div key={label} className="rounded-xl border border-[#eadfd6] bg-[#fffdf9] p-5"><p className="text-[11px] font-bold uppercase tracking-[.12em] text-[#897d77]">{label}</p><p className="mt-2 font-serif text-3xl font-semibold">{loading ? "-" : value}</p></div>)}</section>
