...
import CompactChartLegend from "../components/manager/CompactChartLegend.jsx"
                    ...
                    {isManager && (
                        <>
                            ...
                            <section className="rounded-xl border border-[#eadfd6] bg-[#fffdf9] p-6 sm:p-8">
                                ...
                                <div className="mt-6 h-[330px]">{reportLoading ? <div className="h-full animate-pulse rounded-xl bg-[#faf6ef]" /> : (report?.finance?.periods?.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={report.finance.periods} margin={{ top: 10, right: 12, left: 8, bottom: 10 }}><CartesianGrid stroke="#eadfd6" vertical={false} /><XAxis dataKey="label" tick={{ fill: "#766b66", fontSize: 11 }} minTickGap={20} /><YAxis width={50} tick={{ fill: "#897d77", fontSize: 10 }} tickFormatter={formatAxisCurrency} /><Tooltip formatter={(value, name) => [formatCurrency(value), name]} cursor={{ fill: "#faf6ef" }} /><Legend content={<CompactChartLegend />} height={30} /><Bar name="Doanh thu tiền thuê" dataKey="rentalRevenue" fill="#f2a39b" radius={[4, 4, 0, 0]} /><Bar name="Phí phát sinh" dataKey="additionalFees" fill="#8b4d47" radius={[4, 4, 0, 0]} /><Bar name="Refund" dataKey="refunds" fill="#857371" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer> : <div className="flex h-full items-center justify-center rounded-xl bg-[#faf6ef]/60 text-sm text-[#897d77]">Chưa có dữ liệu tài chính trong khoảng này.</div>)}</div>
